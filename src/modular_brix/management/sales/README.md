...
- Models: `Quote` (versioned and, with its lines, frozen by ORM guards and PostgreSQL triggers once sent), `QuoteLine`, `SalesOrder` (one per quote), `SalesOrderLine` (DB check: delivered <= ordered), `Delivery`, `DeliveryLine`.

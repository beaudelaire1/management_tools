...
- Models: `Invoice` — draft then irreversible issuance; ORM guards make an issued invoice non-modifiable and non-deletable; seller/buyer snapshots frozen at issuance; partial unique constraint on number. `InvoiceLine`, `CreditNote` (positive, capped).
